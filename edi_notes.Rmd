...
jo2<-jo %>% 
  ...
  rename(levels=`Job Openings`) %>% 
  mutate(Series="Job Openings")

un2<-un %>% 
  rename(Date=DATE) %>% 
  mutate("Seeking Employment"=UNEMPLOY+NILFWJN+LNS12032194) %>% 
  select(Date,"Seeking Employment") %>% 
  rename(levels=`Seeking Employment`) %>% 
  filter(Date<="2021-12-01") %>% 
  filter(Date>="2000-12-01") %>% 
    mutate(Series="Seeking employment")


jo_un<-bind_rows(jo2,un2)
```


```{r}
jo_un_chart<-ggplot(jo_un,aes(x=Date,y=levels,color=Series))+
  geom_rect(xmin=as.Date("2020-02-01"), xmax=as.Date("2020-04-01"), 
            ymin=0, ymax=Inf, fill="grey90", alpha=0.3, col="grey90")+
  geom_line(size=1)+
  scale_color_manual(
    values=c("#192954","#be2b29"))+
  labs(title="Not enough job openings for all those seeking employment",subtitle ="Job openings with unemployed + NILF want a job now + PT for economic reasons, Dec 2000 - Dec 2021", caption = "U.S. BLS, retrieved from FRED, Federal Reserve Bank of St. Louis",y="Level in thousands",x="Date (monthly)")+
    scale_y_continuous(label=comma)+
  theme_bw()+
  scale_x_date(breaks = pretty_breaks(11))+
    theme(legend.position="bottom",legend.title = element_blank())
...
jo_un_chart
ggsave("job_openings_wantwork_chart.png",width=8,height=3.2)

```

```{r}
newburgh_urate<-read_csv(here("newburgh_urate_bls.csv"))
```

```{r}
urate<-newburgh_urate %>% 
  pivot_longer(2:4,names_to="Series",values_to="U-3")
```
...
u3_chart<-ggplot(urate,aes(x=Date,y=`U-3`,color=Series))+
  ...
  geom_line(size=1)+
  scale_color_manual(
    values=c("#192954","#be2b29","#fdcf60"))+
  labs(title="Unemployment rate comparison",subtitle ="Jan 2019 - Dec 2021", caption = "U.S. BLS, retrieved from mySidewalk Inc",y="Unemployment rate (%)",x="Date (monthly)")+
    scale_y_continuous(label=comma)+
  theme_bw()+
...
u3_chart
ggsave("newburgh_u3comp_chart.png",width=8,height=3.5)
